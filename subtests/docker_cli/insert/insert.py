...
class insert(subtest.Subtest):
    ...
    def initialize(self):
        ...
        #note current containers to help clean up later
        if self.config['remove_after_test']:
            containers = DockerContainers(self, 'cli').get_container_list()
            safe_ids = [x.long_id for x in containers]
            self.stuff['safe_ids'] = safe_ids
    ...
    def cleanup(self):
        if self.config['remove_after_test']:
            ...
            subargs = filter(lambda x: x not in self.stuff['safe_ids'], ids)
            dkrcmd = DockerCmd(self, 'rm', subargs)
            dkrcmd.execute()
